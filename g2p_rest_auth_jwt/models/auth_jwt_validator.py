...
class AuthJWTValidator(models.Model):
    _name = "g2p.auth.jwt.validator"
    _description = "JWT Validator Configuration"

    @api.model
    def _default_user_id(self):
        retval = None
        if self.env.ref("g2p_registry_rest_api.default_api_user"):
            retval = self.env.ref("g2p_registry_rest_api.default_api_user").id
        return retval
    ...
    default_user_id = fields.Many2one(
        "res.users", default=_default_user_id, required=True
    )
