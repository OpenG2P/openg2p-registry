...
class TestBulkOperations(TransactionCase):
    ...
    def test_bulk_approve_wizard_creation(self):
        ...
        wizard = self.env['change.request.bulk.wizard'].with_context(
            active_ids=[self.change_request_3.id],
    ...
    def test_bulk_submit_wizard_creation(self):
        ...
        wizard = self.env['change.request.bulk.wizard'].with_context(
            active_ids=[self.change_request_1.id, self.change_request_2.id],
    ...
    def test_bulk_assign_wizard(self):
        ...
        wizard = self.env['change.request.bulk.wizard'].with_context(
            ...
            'assign_to_user_id': self.approver.id,
