...
def test_upload_image_less_than_1mb(self):
    ...
    self.assertEqual(self.partner.image_1920, img_str, "Image should be stored correctly in the database")
    # to check if no file is stored in the S3 bucket
...
def test_upload_image_more_than_1mb(self):
    img_str = self.create_image(3000)
    self.partner.write({"image_1920": img_str})
    ...
    self.assertEqual(storage_file.name, "Profile Image", "Stored file should have the tag 'Profile Image'")
    self.assertEqual(
...
def test_delete_s3_file_on_image_delete(self):
    ...
    storage_files = self.env["storage.file"].search([("registrant_id", "=", self.partner.id)])
    self.assertFalse(storage_files, "Corresponding S3 entry should be deleted when profile image is removed")
